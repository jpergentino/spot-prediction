...
public final class CBREngine {
	...
	private static CBREngine uniqueInstance = null;
	...
	public synchronized boolean addCase(List<Case> caseList)  {
		
		boolean add = true;
		
		for (Case caseObj : caseList) {
			if (!addCase(caseObj)) {
				log.debug("Case rejected "+ caseObj);
				add = false;
				break;
			}
		}
...
//			i.getAttributes().entrySet().stream().forEach(x -> log.debug("Adding: "+ x.getKey().getName() +"="+ x.getValue().getValueAsString()));
	...
	public boolean saveCaseDatabaseToFile(File file) {
		...
		log.debug("Saving CBR Database file to "+ file.getAbsolutePath());
		...
		List<String> lines = cases.stream().map(x -> new Case().fromInstance(x).toStringCase()).collect(Collectors.toList());
		...
		log.debug("Successfully saved "+ cases.size() +" cases to "+ file.getAbsolutePath());
		return ret;
	...
	public List<Pair<Instance, Similarity>> query(Map<String, Object> attrMap, boolean returnAll) throws ParseException {
		
		Retrieval r = new Retrieval(concept, database);
		r.setRetrievalMethod(returnAll ? RetrievalMethod.RETRIEVE_SORTED : RetrievalMethod.RETRIEVE_K_SORTED);
...
//		}));
	...
	public void importCsvInstances(File file) {
		...
		imp.setCaseBase(p.getCB(CASEBASE_DATABASE_NAME));
		imp.readData();
		imp.checkData();
